/**
 * movit-share — the distribution primitive for Movit.
 *
 * Encodes a `.movit` document into a compact, URL-safe token so any movement
 * can travel as a link and render wherever it lands. This is the mechanic that
 * lets Movit spread the way Mermaid did: a document *is* a shareable URL.
 *
...
/** Encode a `.movit` document into a URL-safe token. Throws on invalid input. */
export function encodeMovit(source: string): string {
  if (typeof source !== "string" || source.length === 0) {
    throw new TypeError("encodeMovit: source must be a non-empty string");
  }
  if (source.length > MAX_SOURCE_LENGTH) {
    throw new RangeError(
      `encodeMovit: document is ${source.length} chars, over the ${MAX_SOURCE_LENGTH} limit for a shareable link`,
    );
  }
...
export function decodeMovit(token: string): string {
  if (typeof token !== "string" || !BASE64URL.test(token)) {
    throw new TypeError("decodeMovit: token is not a valid base64url string");
  }
...
export function buildShareHash(source: string): string {
  return `#${SHARE_PARAM}=${encodeMovit(source)}`;
}
...
// Fully self-contained: no TextEncoder/atob/Buffer. This keeps movit-share a
...
function base64UrlToBytes(token: string): number[] {
  ...
  for (let i = 0; i < token.length; i++) {
    ...
    if (value < 0) {
      throw new Error("decodeMovit: token contains a non-base64url character");
    }
