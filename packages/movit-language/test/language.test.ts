...
describe("getCompletions", () => {
  ...
  const onLine = (lineText: string, ch: number) =>
    getCompletions(`movit exercise "x"\n${lineText}`, 1, ch).map((c) => c.label);

  it("suggests movement kinds right after `movit `", () => {
    expect(getCompletions("movit ", 0, 6).map((c) => c.label)).toEqual(
